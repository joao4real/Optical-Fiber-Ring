...
gama=1.3174e-3;       % Coeficiente de nao-linearidade da fibra
alfa_dB_km=0.22;      % coeficiente de atenuacao em dB
L_sec= cL_sec*1e3;       % Comprimento da seccao de fibra em m
CR=0.028*1e-15;     % Slope do ganho de Raman (da aproximação triangular
%CR=0;      %% Without SRS
D_lambda=17.108e-6;    % ParÃ¢metro de dispersao da fibra (s/m^2);
S_lambda=0.088*1e3;  % Declive da dispersao da fibra (s/m^3);
%S_lambda=0;
Nch=cN_ch;             % Numero de canais WDM
Espac_canais=cCh_sp*1e9;          %%% Espacamento entre canais de referencia
P_ch_ref_dBm=txrx(4);
f_inicial=c_Lv*1e12;   % Frequencia inicial da banda C
R_sl=c_Rsl*1e9;          %% Ritmo de símbolo
